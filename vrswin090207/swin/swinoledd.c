...
#
# swinoledd.c
# Modified and programmed by nyasu  
# (C) 2002 "Nishikawa,Yasuhiro" <nyasu@osk.3web.ne.jp>
#
#  Original Copyright:
#     OleDragDrop.h and OleDragDrop.c
###   (C) Copyright 1996-1999 By Tomoaki Nakashima. All right reserved.
###        http://www.nakka.com/
###        nakka@nakka.com
#
# He states at his web page that the codes are free to be used and embedded.
# nyasu@osk.3web added some definition for Cygwin and replaced comments 
# as c-style (not //) .
#
#
###################################
...
#include <ole2.h>   /* my cygwin used to need this */
#endif
...
#ifdef __CYGWIN__
#ifndef MK_ALT      /* my cygwin headers don't know about this. */
#define MK_ALT 0x20
#endif
...
#if __BORLANDC__
	pdtn->hMem = sm.u.hGlobal; /* not _ANONYMOUS_UNION in bcc. #by yukimi_sake */
#else
...
#if __BORLANDC__
  pmedium->u.hGlobal = hMem; /* not _ANONYMOUS_UNION in bcc. #by yukimi_sake */
#else
...
static VALUE
swin_oledndtargetstart(VALUE obj,VALUE msg_id, VALUE formats){
	int nFormat;
	int* cFormats;
	struct Swin *sw;
	int i,r;

	if(TYPE(formats)!=T_ARRAY){
		rb_raise(rb_eArgError,"cf formats is not an array.");
		return Qnil;
	}
	nFormat = RARRAY_LEN(formats);

	MYMALLOC(cFormats,sizeof(int)*(nFormat),Qnil);

	for(i=0;i<nFormat;i++){
		cFormats[i] = NUM2INT( rb_ary_entry(formats,i) ); 
	}
	Data_Get_Struct(obj, struct Swin, sw);
	r = OLE_IDropTarget_RegisterDragDrop(sw->hWnd,NUM2UINT(msg_id),
				cFormats,nFormat);
	return INT2NUM(r);
}

static VALUE
swin_oledndtargetstop(VALUE obj){
	struct Swin *sw;
	int r;
	Data_Get_Struct(obj, struct Swin, sw);
	OLE_IDropTarget_RevokeDragDrop(sw->hWnd);

	return obj;
}

static VALUE
swin_oledragstart(VALUE obj, VALUE msg_id, VALUE formats, VALUE efct){
	int nFormat;
	int* cFormats;
	int effect;
	struct Swin *sw;
	...
	Data_Get_Struct(obj, struct Swin, sw);
	r = OLE_IDropSource_Start(sw->hWnd,
				NUM2INT(msg_id),cFormats,nFormat,effect );
	return INT2NUM(r);
}
...
void Init_swinolednd(){
	...
	switch(r){
	case S_OK:
		atexit((void (*)(void))OleUninitialize);
		break;
	case S_FALSE:
		/* nothing necessary to do*/
		break;
	default:
		rb_raise(rb_eRuntimeError,"OLE Initialization failed(%d)",r);
		break;
	}
}
